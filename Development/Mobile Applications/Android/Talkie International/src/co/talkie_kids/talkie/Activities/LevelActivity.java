...
public class LevelActivity extends BaseTalkieActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		startSound(mTalkieObj.getSound(this.mIndex),
				mOnFirstPlayerCompletionListener);

		((RelativeLayout) findViewById(R.id.rlMic))
				.setOnClickListener(mOnMicClickListener);
	}
	...
	class WordRecognitionListener implements RecognitionListener {
		...
		public void onRmsChanged(float rmsdB) {
			...
			/*
			 * short newDbLevel; if( rmsdB < - 75 ) { newDbLevel = 0; } else if(
			 * rmsdB < -30 ) { newDbLevel = 1; } else if(rmsdB < 15) {
			 * newDbLevel = 2; } else { newDbLevel = 3; }
			 */
		...
		public void onResults(Bundle results) {
			...
			if (data.contains(obj.getWord(mIndex))) {
				...
				startSound(R.raw.right, mOnSRPlayerCompletionListener);
				...
				JsonDataParser jpars = new JsonDataParser(
		...
		MediaPlayer.OnCompletionListener mOnSRPlayerCompletionListener = new MediaPlayer.OnCompletionListener() {
			...
			public void onCompletion(MediaPlayer mp) {
				...
				if (!mIsPaused) {
					...
					if (isCorrect) {
						if ((mIndex + 1) % 3 == 0) {
							if (!obj.getKey().equalsIgnoreCase(
									FruitsAndVeggies.FRU_AND_VEGGY_KEY)) {
								...
								if (mIndex >= obj.getCount() - 1) {
									...
									if (BaseTalkieActivity.mMediaPlayer
											.isPlaying())
										BaseTalkieActivity.mMediaPlayer.stop();
									...
									/*
									 * sr.stopListening(); sr.cancel();
									 */

									ImageView animal = (ImageView) findViewById(R.id.animal);

									animal.setImageResource(obj
											.getDrawable(mIndex));
						 ...
						 * 
						 * //Log.d("shit test", "not correct "); Intent intent =
	...
	MediaPlayer.OnCompletionListener mOnFirstPlayerCompletionListener = new MediaPlayer.OnCompletionListener() {
		@Override
		public void onCompletion(MediaPlayer mp) {
			// Log.v("zzzzz", Boolean.toString(mIsPaused));
			if (!mIsPaused) {
	...
	protected MediaPlayer startSound(int resId,
			MediaPlayer.OnCompletionListener listener) {
		MediaPlayer mp = MediaPlayerOptimizer.create(LevelActivity.this, resId,
				false);
	...
	public void onPause() {
		...
		for (Object mediaPlayer : mMediaPlayers.toArray()) {
			stopSound((MediaPlayer) mediaPlayer); // stop, release, and free for
													// GC, each mp.
		}
	...
	public void onResume() {
		...
		mWordRecognitionListener = new WordRecognitionListener(this.mIndex,
				mTalkieObj);
	...
	class MyOnErrorListener implements OnErrorListener {
		...
		MyOnErrorListener(int resourceId,
