...
public class GameActivity extends BaseTalkieActivity {
		...
		if (mFallinObjectsCount <= mMaximumFallingObjects) {
			...
			try {
				long milSecToDelay = mMinDropNewObjectDelayMiliseconds
						+ ((long) (mRand.nextDouble() * (mMaxDropNewObjectDelayMiliseconds - mMinDropNewObjectDelayMiliseconds)));
				...
				Log.v("error", "-----------------------------------------");
				Log.v("mMinDropNewObjectDelayMiliseconds",
						Long.toString(mMinDropNewObjectDelayMiliseconds));
				Log.v("mMaxDropNewObjectDelayMiliseconds",
						Long.toString(mMaxDropNewObjectDelayMiliseconds));
				Log.v("end error", "-----------------------------------------");
			}
		}
	}
	...
	private void dropObject() {
		...
		final int fallenObjectListIndex = mRand
				.nextInt(FALLING_OBJECTS_LAYOUTS_LIST.size());
		final ImageView fallingObject = (ImageView) getLayoutInflater()
				.inflate(
						FALLING_OBJECTS_LAYOUTS_LIST.get(fallenObjectListIndex),
						null);
		...
		RelativeLayout.LayoutParams params = (RelativeLayout.LayoutParams) fallingObject
				.getLayoutParams();
		...
		vto.addOnGlobalLayoutListener(new OnGlobalLayoutListener() {
			...
			public void onGlobalLayout() {
				...
				final int newLeftMargin = rlColumn.getHeight()
				...
				final Animation a = new Animation() {
					...
					protected void applyTransformation(float interpolatedTime,
						...
						RelativeLayout.LayoutParams params = (RelativeLayout.LayoutParams) fallingObject
								.getLayoutParams();
						params.topMargin = (int) (newLeftMargin * interpolatedTime)
								- fallingObject.getHeight();
						params.addRule(RelativeLayout.CENTER_HORIZONTAL);
						fallingObject.setLayoutParams(params);

						if (params.topMargin >= newLeftMargin
								- mIvCharacter.getHeight()
								- fallingObject.getHeight()) {
							this.cancel();

							if (((RelativeLayout) mIvCharacter.getParent())
									.getId() == rlColumn.getId())
				 ...
				 * R.anim.fadein); fadein.setDuration(10);
				 ...
				 * fallingObject.getLayoutParams(); params.topMargin = -
				 * fallingObject.getHeight();
				 * params.addRule(RelativeLayout.CENTER_HORIZONTAL);
				 * fallingObject.setLayoutParams(params); }
				 * 
	...
	protected void goLeft() {
		...
		if (columnId != COLUMN_IDS_LIST.get(0) && columnId != View.NO_ID) {
			moveCharacterTo(COLUMN_IDS_LIST.get(COLUMN_IDS_LIST
					.indexOf(columnId) - 1));
		}
	}
	...
	protected void eatObject(int fallenObjectListIndex) {
		...
		if (fallenObjectListIndex == 0) {
			...
			/*
			 * //if( mMaxDropNewObjectDelayMiliseconds >
			 * mMinDropNewObjectDelayMiliseconds) {
			 * mMaxDropNewObjectDelayMiliseconds += SCORE_PER_STAR
			 * (mMaxDropNewObjectDelayMiliseconds / mMaxFallingTimeMiliseconds)
			 * * 10;
			 * 
			 ...
			 * mMinDropNewObjectDelayMiliseconds += increaseMinTimeWith; } else
			...
			long score = Long.valueOf(tvScore.getText().toString())
					+ SCORE_PER_STAR;

			if (score > 6000) {
				Log.v("log", "-----------------------------------------");
				Log.v("mMinFallingTimeMiliseconds",
						Long.toString(mMinFallingTimeMiliseconds));
				Log.v("mMaxFallingTimeMiliseconds",
						Long.toString(mMaxFallingTimeMiliseconds));
				Log.v("mMinDropNewObjectDelayMiliseconds",
						Long.toString(mMinDropNewObjectDelayMiliseconds));
				Log.v("mMaxDropNewObjectDelayMiliseconds",
						Long.toString(mMaxDropNewObjectDelayMiliseconds));
				Log.v("mMaximumFallingObjects",
						Long.toString(mMaximumFallingObjects));
				Log.v("mLimitFallingObjects",
						Long.toString(mLimitFallingObjects));
	...
	private void gameOver() {
		...
		Toast.makeText(getApplicationContext(),
				Integer.toString(mMaxFallingCount), Toast.LENGTH_SHORT).show();
