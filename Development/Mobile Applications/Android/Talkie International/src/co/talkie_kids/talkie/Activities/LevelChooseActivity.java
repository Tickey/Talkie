...
public class LevelChooseActivity extends BaseTalkieActivity {
	...
	private void setupGameViews() {
		...
		for (int i = 0; i < 3; i++) {
			View currentView = v.findViewWithTag("anim-"
					+ Integer.toString(i + 1));
			currentView.setClickable(true);

			if (i < jparser.getUnlockedAnimationsNumber(mCategoryKey)
					&& !mCategoryKey
							.equalsIgnoreCase(FruitsAndVeggies.FRU_AND_VEGGY_KEY)) {
				ImageView img = (ImageView) currentView
